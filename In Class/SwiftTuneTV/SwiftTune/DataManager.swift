...
class DataManager: NSObject {
    ...
    func fileIsLocal(filename :String) -> Bool {
        ...
        #if os(tvOS)
        return fileManager.fileExistsAtPath(getCachesPathForFile(filename))  //using getdocumentpathforfile to see if said filename exists in the directory
        #else
    ...
    func getCachesPathForFile(filename: String) -> String {
        let cachesPath = NSSearchPathForDirectoriesInDomains(.CachesDirectory, .UserDomainMask, true)[0] as NSString       //no documents directory in apple tv, need cache instead
    ...
    func getImageFromServer(localFilename: String, remoteFilename: String, indexPathRow: Int) {
        ...
        if let _ = imageTemp {
            #if os(tvOS)
                imageData!.writeToFile(getCachesPathForFile(localFilename), atomically: false)
            #else
...
//            print(jsonResult)
    ...
    func getDataFromServer(searchTerm: String) {
...
//        }
        let url = NSURL(string: "http://\(baseURLString)/search?term=\(searchTerm)")
